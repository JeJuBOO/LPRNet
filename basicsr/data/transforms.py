...
def paired_psnr_random_crop(img_gts, img_lqs, gt_patch_size, scale, gt_path, num_samples=4):
    """Paired random crop with PSNR-based selection of the lowest quality patch.
    ...
    Args:
        ...
        scale (int): Scale factor.
        gt_path (str): Path to ground-truth.
        num_samples (int): Number of random crops to try.

    Returns:
        list[ndarray] | ndarray: Selected GT and LQ image patches with lowest PSNR.
    """
    ...
    selected_lqs, selected_gts = None, None
    ...
    for _ in range(num_samples):
        ...
        cropped_lqs = [v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_lqs]
        cropped_gts = [v[top_gt:top_gt + gt_patch_size, left_gt:left_gt + gt_patch_size, ...] for v in img_gts]


        # PSNR 계산 (첫 번째 쌍만 기준)
        psnr = compare_psnr(cropped_gts[0], cropped_lqs[0])
        if psnr < lowest_psnr:
            lowest_psnr = psnr
            selected_lqs, selected_gts = cropped_lqs, cropped_gts

    if len(selected_gts) == 1:
        selected_gts = selected_gts[0]
    if len(selected_lqs) == 1:
        selected_lqs = selected_lqs[0]
    return selected_gts, selected_lqs