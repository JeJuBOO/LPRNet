...
def check_specific_parameter(model):
...
# export PYTHONPATH=/home/work/image_restoration/TSMPNet
# torchrun --nproc_per_node=2 --master_port=1234 basicsr/train.py -opt options/train/GoPro/TSMKDNet-width64.yml --launcher pytorch

def main():
    wandb.init(
        project="TSMP",
        resume="allow",
        name="Sub_ablation_v5_addinter_new",
        id="v3jbjy0q",
        #mode="disabled"
        )
    # 학습할때 이거 False로 바꿔야함
    ...
    is_pretrain = opt['datasets']['train']['is_pretrain']
    freeze_param = opt['datasets']['train']['is_freeze']
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            result_code = model.optimize_parameters(current_iter, tb_logger, is_pretrain, freeze_param)
            ...
            if current_iter % opt['logger']['print_freq'] == 0:
                ...
                msg_logger(log_vars)
                if wandb.run is not None:  # wandb가 초기화됐는지 확인
                    wandb.log({
                        "epoch": epoch,
                        "iter": current_iter,
                        "loss": model.get_current_log(),
                        "learning_rate": model.get_current_learning_rate()[0]
                    }, step=current_iter)

            #check_specific_parameter(model)
            ...
            if opt.get('val') is not None and (current_iter % opt['val']['val_freq'] == 0):# or current_iter == 1000):
                ...
                if dist.get_rank() == 0 and wandb.run is not None:
                    ...
                    if 'psnr' in metric:
                        log_data["val/psnr"] = metric['psnr']
                    if 'ssim' in metric:
                        log_data["val/ssim"] = metric['ssim']

                    wandb.log(log_data, step=current_iter)

                # PSNR과 SSIM 모두 개선된 경우에만 저장
                if 'psnr' in metric:
                    if metric['psnr'] > best_psnr and metric['ssim'] > best_ssim:
                        best_psnr = metric['psnr']
                        best_ssim = metric['ssim']
                        logger.info(
                            f'Saving best model at iteration {current_iter}, PSNR: {best_psnr:.4f}, SSIM: {best_ssim:.4f}')
                        model.save(epoch, current_iter)
                del metric
                torch.cuda.empty_cache()
            data_time = time.time()
