...
class BaseModel():
    ...
    def model_to_device(self, net):
        ...
        if self.opt['dist']:
            ...
            net = DistributedDataParallel(
                ...
                broadcast_buffers=False,  # 버퍼 동기화 OFF
                static_graph=True,         # 정적 그래프 명시
                find_unused_parameters=False,  # 미사용 파라미터 검색 OFF
    ...
    #         # param_key가 존재하면 해당 키 사용
    #         if param_key in load_net:
    #             load_net = load_net[param_key]
    #         # param_key가 없고 'params'인 경우 경고 후 전체 사용
    #         elif param_key == 'params':
    #             logger.warning(f'⚠️ "{param_key}" key not found. Using entire checkpoint.')
    #         else:
    #             # 지정된 키가 없는 경우 예외 발생
    #             raise KeyError(f'Key "{param_key}" not found in checkpoint')
    #     # param_key=None인 경우 전체 체크포인트 사용
        
    #     # 3. 키 확인 및 디버깅 정보 출력
    #     print(f'Loaded keys count: {len(load_net.keys())}')
    #     if len(load_net.keys()) > 0:
    #         sample_key = next(iter(load_net.keys()))
    ...
    #             new_key = k[7:]
    #             load_net_copy[new_key] = v
    #         else:
    #             load_net_copy[k] = v
        
    #     # 5. 키 매칭 검증
    #     model_keys = net.state_dict().keys()
    #     load_keys = load_net_copy.keys()
        
    #     missing_keys = [k for k in model_keys if k not in load_keys]
    #     unexpected_keys = [k for k in load_keys if k not in model_keys]
        
    #     print(f'Missing keys: {len(missing_keys)}')
    #     print(f'Unexpected keys: {len(unexpected_keys)}')
        
    #     if missing_keys and strict:
    #         raise RuntimeError(f'Missing keys in checkpoint: {missing_keys[:5]}...')
        
    #     # 6. 최종 로딩
    #     net.load_state_dict(load_net_copy, strict=False)
