...
class DPSABlock(nn.Module):
    def __init__(self, c,  dwconv_kernel_size=7, NAFNetmode=False ,FFN_Expand=2, drop_out_rate=0.):
        super().__init__()
...
class Patched_DPSABlock(nn.Module):
    def __init__(self, channel, patch_size, dwconv_kernel_size=7, NAFNetmode=False, FirstMode=False, patch_skip = False, global_skip = False, FFN_Expand=2, drop_out_rate=0., attn_drop=0.):
        ...
        self.beta = nn.Parameter(torch.zeros((1, c, 1, 1)), requires_grad=True)
        self.gamma = nn.Parameter(torch.zeros((1, c, 1, 1)), requires_grad=True)
        self.ca_params = nn.Parameter(torch.ones(1, c, 1, 1), requires_grad=True)
    ...
    def forward(self, inp):
        ...
        if self.nafnetmode:
            local_features = self.local_path(x)  # x: (B, C, H, W)
            combined_feature = local_features
        else:
            local_features = self.local_path(x) # x: (B, C, H, W)
            global_features = self.global_path(x)

            # local_features_ = self.ln(local_features.permute(0,2,3,1)).permute(0,3,1,2) # x: (B, C, H, W)
            # global_features_ = self.ln(global_features.permute(0,2,3,1)).permute(0,3,1,2) # x: (B, C, H, W)

            # ca_params = self.sigmoid(self.ca_params)
            
            # CA_feature = local_features_ * ca_params + global_features_ * (1-ca_params) # x: (B, C, H, W)
            combined_feature = torch.cat([local_features, global_features], dim=1) # x: (B, 2C, H, W)
            combined_feature = self.norm_dw(combined_feature) # x: (B, 2C, H, W)
            combined_feature = self.conv_dw(combined_feature) # x: (B, 2C, H, W)
        ...
        x_pat1 = self.dconv1(x_pat)  # (B*N, C, wH, wW)
        ...
        x_pat2 = self.dconv2(x_pat)  # (B*N, C, wH, wW)
...
class Patched_DPSANet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=32, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], patch_size=[16, 16], dwconv_kernel_size=5):
        super().__init__()
        self.patch_size = patch_size

        # self.preprocessing = Patched_DPSABlock(img_channel,
        #                                        patch_size,
        ...
        for i in range(len(enc_blk_nums)):
            ...
            chan = chan * 2
            self.patch_size = [size // 2 for size in self.patch_size]
