...
class MDBlock(nn.Module):
    def __init__(self, c, dwconv_kernel_size, patch_size, 
                 NAFNetmode=False, 
                 Patchmode=False, 
                 FFTmode=False, 
        ...
        if FFTmode:
            self.fft = fft_bench_complex_conv(c, dw=1, act_method=nn.GELU())
            self.fft2 = fft_bench_complex_conv(c, dw=1, act_method=nn.GELU())
    ...
    def forward(self, inp):
        ...
        if self.fftmode:
            fft_ft = self.fft(x)
            if self.patchmode:
                fft_ft = patch_reverse(fft_ft, H, W)
        ...
        if self.patchmode:
            feature = patch_reverse(feature, H, W)
...
class PatchBlock(nn.Module):
    ...
    def forward(self, x, first_stage_feat):
        ...
        if self.fftmode:
            fft_feature = self.fft(x)
            fft_feature = patch_reverse(fft_feature, H, W)
...
class TSMKDNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=32, first_middle_blk_num=1, first_enc_blk_nums=[], first_dec_blk_nums=[],
        ...
        self.run_second_stage_middle_blks = nn.ModuleList()
        ...
        for i in range(len(first_dec_blk_nums)):
            ...
            self.run_first_stage_decoders.append(
                nn.Sequential(
                    *[MDBlock(chan, dwconv_kernel_size=7, patch_size=self.patch_size, 
                              Patchmode=False,
                              FFTmode=False) for _ in range(first_dec_blk_nums[i])]
                )
            )

        self.run_sam = SAM(chan, width)
        ...
        for i in range(len(second_enc_blk_nums)):
            self.run_second_stage_encoders.append(
                nn.Sequential(
                    *[PatchBlock(chan, self.patch_size, 
                                 FFTmode=False) for _ in range(second_enc_blk_nums[i])]
                )
            )
        ...
        for i in range(len(second_dec_blk_nums)):
            ...
            chan = chan // 2
            self.patch_size = self.patch_size * 2
            self.run_fft_interaction.append(
                nn.Sequential(
                    fft_bench_complex_conv(chan, dw=1, act_method=nn.GELU())
                )
            )
            self.run_second_stage_decoders.append(
                nn.Sequential(
                    *[MDBlock(chan, dwconv_kernel_size=7, patch_size=self.patch_size, 
                              Patchmode=True,
                              FFTmode=False) for _ in range(second_dec_blk_nums[i])]
                )
            )
    ...
    def forward(self, inp):
        ...
        first_feature, first_out_img = self.run_sam(x, inp)
        ...
        for decoder, up, enc_skip, fir_skip, fft_inter in zip(self.run_second_stage_decoders, self.run_second_stage_ups, encs2[::-1],
                                                   first_dec_features, self.run_fft_interaction):
            x = up(x)
            # x = x + enc_skip
            for block in decoder:
                enc_skip = enc_skip + fft_inter(enc_skip)
                x = x + enc_skip
                x = block(x)
